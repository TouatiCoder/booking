...
if ($method === 'GET') {
    if (isset($_GET['action']) && $_GET['action'] == 'can_review') {
        ...
        $stmt = $conn->prepare("
            SELECT b.id FROM guide_reservations b
            WHERE b.client_id = ? AND b.guide_id = ? AND b.reservation_status = 'completed' AND b.payment_status = 'paid'
            AND b.id NOT IN (SELECT reservation_id FROM guide_reviews WHERE user_id = ?)
            LIMIT 1
        ");
        ...
        $stmt = $conn->prepare("SELECT r.*, u.full_name as user_name FROM guide_reviews r JOIN users u ON r.user_id = u.id WHERE r.guide_id = ? ORDER BY r.created_at DESC");
    ...
    $id = uniqid("gr_");
    $stmt = $conn->prepare("INSERT INTO guide_reviews (id, guide_id, user_id, reservation_id, rating, comment) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->execute([$id, $data->guide_id, $data->user_id, $data->booking_id, $data->rating, $data->comment]);

    // Recalculate guide rating
    $stmt = $conn->prepare("SELECT AVG(rating) as avg_rating, COUNT(id) as total FROM guide_reviews WHERE guide_id = ?");
